...
from gui.utils import do_create_movie, do_get_movie, do_delete_movie, do_get_paginated_movie, do_logout
import json
...
def create():
    ...
    if request.method == 'GET':
        ...
        if not isinstance(length, int):
            flash('Длительность должна быть введена в минутах', 'error')
            return redirect(url_for('movies.create'))
        result = do_create_movie(name, description, length, request.cookies)
        if result.success:
...
def get():
    ...
    if request.method == 'GET':
        ...
        if 'movie_id' not in request.form or request.form['movie_id'] == '':
            ...
            result = do_get_movie(movie_id, request.cookies)
...
def delete(movie_id):
    ...
    if request.method == 'GET':
        ...
        if request.form['submit'] == 'Да':
            ...
            result = do_delete_movie(movie_id, request.cookies)
...
def get_all():
    ...
    if request.method == 'GET':
        ...
        result = do_get_paginated_movie(page, 10, request.cookies)
        if result.success:
            if result.response.status_code == 200:
